...
def enc_dec_mask(device, dataset, T, S):
    mask = torch.ones(T, S)
    #modelmead的逻辑与BIWI保持一致：每个顶点帧对应两个连续的音频步，确保时序对齐，避免cross-attention中的数值不稳定。
    if dataset == ".":
        for i in range(T):
            start = i * 2
            if start >= S:
...
class Faceformer(nn.Module):
    def __init__(self, args):
        ...
        self.audio_encoder = Wav2Vec2Model.from_pretrained("/home/tkface/workspace/FaceFormer/wav2vec2-base-960h")
    ...
    def forward(self, audio, template, vertice, one_hot, criterion,teacher_forcing=True):
        ...
        # 与 BIWI 对齐策略一致：保持 2:1 音频步与顶点帧关系，避免时序错位。
        if self.dataset == ".":
